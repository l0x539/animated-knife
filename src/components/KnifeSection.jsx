...
const KnifeSection = (props) => {
  ...
  useEffect(() => {
    // Set initial translateY to a large negative value to start from off-screen bottom
    gsap.set(settings, { translateY: -3 }); // Adjust -3 as needed
    ...
    gsap.to(settings, {
      scrollTrigger: {
        ...
        markers: TESTING,
        onUpdate: (self) => {
          // Animate the knife upwards as the user scrolls down
          const translateYValue = (isTablet || isMobile) ? -0.1 : -0.3; // Adjust these values as needed
          gsap.to(settings, {
            translateY: self.progress > 0 ? translateYValue : -3, // -3 to reset to off-screen bottom
            ease: "power1.out",
          });
  ...
  useFrame(() => {
    if (mixerAction1 && mixerAction2 && duration) {
      ...
      // Calculate the translateY value to position the knife at the screen's center
      const centerYPosition = isMobile ? 0.55 : 0.3; // This is the center position
      const offScreenBottomPosition = isMobile ? -0.55 : -0.6; // This is the offscreen position
      const translateY = gsap.utils.interpolate(
        offScreenBottomPosition,
        centerYPosition,
        settings.progress
      );

      knife.current.position.setY(translateY);
    }
  });
